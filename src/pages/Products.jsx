...
const Products = () => {
  ...
  const [form, setForm] = useState({ name: '', sku: '', description: '', base_price: '', vendor_price: '', retailer_price: '', gst_rate: '18', stock_qty: '0', low_stock_threshold: '10', category_id: '', unit: 'piece', image_url: '' });
  ...
  const openCreate = () => {
    setEditItem(null);
    setForm({ name: '', sku: '', description: '', base_price: '', vendor_price: '', retailer_price: '', gst_rate: '18', stock_qty: '0', low_stock_threshold: '10', category_id: categories[0]?.id || '', unit: 'piece', image_url: '' });
    setShowModal(true);
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      // Do NOT set Content-Type manually — axios auto-sets it with the correct boundary for FormData
    ...
    <div>
      ...
      <div className="table-container">
        ...
        <table className="custom-table">
          <thead><tr><th>Product</th><th>SKU</th><th>Category</th><th>Base Price</th><th>Vendor Price</th><th>Retailer Price</th><th>GST</th><th>Stock</th><th>Actions</th></tr></thead>
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
            ...
            {products.length === 0 && <tr><td colSpan="9" style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>No products found</td></tr>}
          </tbody>
        </table>
      </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  <div className="form-group"><label className="form-label">Unit</label><input className="form-input" value={form.unit} onChange={e => setForm({ ...form, unit: e.target.value })} placeholder="piece" required /></div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div className="form-group"><label className="form-label">Vendor Price (paise)</label><input className="form-input" type="number" value={form.vendor_price} onChange={e => setForm({ ...form, vendor_price: e.target.value })} placeholder="Fallback to Base Price" /></div>
                  <div className="form-group"><label className="form-label">Retailer Price (paise)</label><input className="form-input" type="number" value={form.retailer_price} onChange={e => setForm({ ...form, retailer_price: e.target.value })} placeholder="Fallback to Base Price" /></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
